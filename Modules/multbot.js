var BotGrepo = class {
    ...
    _safeInit = (name, factory) => {
        try {
            ...
            const msg = `[BotGrepo] ✗ Failed to initialize module "${name}": ${e?.message ?? e}`;
            console.error(msg, e);
            try {
    ...
    settingsStatus = () => this.statusPanel ? this.statusPanel.settings() : this._missingModuleHtml('Status');
    ...
    settingsSniper = () => this.sniper ? this.sniper.settings() : this._missingModuleHtml('Sniper');
    settingsSpells = () => this.autoSpells ? this.autoSpells.settings() : this._missingModuleHtml('Auto Spells');
    settingsTrain  = () => this.autoTrain ? this.autoTrain.settings() : this._missingModuleHtml('Auto Train');
    ...
    settingsTrade = () => this.autoTrade ? this.autoTrade.settings() : this._missingModuleHtml('Auto Trade');
    ...
    setup = () => {
        ...
        // Botão da engrenagem — abre BotGrepo
        uw.$('.gods_area_buttons').append(`
            <div class='circle_button bot_greco_settings' onclick='window.botGrepo.settingsFactory.openWindow()'>
                <div style='width: 27px; height: 27px; display:flex; align-items:center; justify-content:center;' class='icon js-caption' title='BotGrepo'>
        ...
        const editController = () => {
            ...
            townController.controller.town_groups_list_view.render = function () {
                ...
                const townIds = uw.botGrepo.autoBuild ? Object.keys(uw.botGrepo.autoBuild.towns_buildings) : [];
                const troopsIds = uw.botGrepo.autoTrain ? uw.botGrepo.autoTrain.getActiveList().map(entry => entry.toString()) : [];
    ...
    createMultMenu = () => {
        const $menu = uw.$('<div id="bot_greco_menu" class="toolbar_activities"></div>');
...
if (!window.__botgrepo_loaded__) {
    window.__botgrepo_loaded__ = true;
    var _botgrepo_loader = setInterval(() => {
        if (uw.$("#loader").length > 0) return;
        // Instancia com o novo nome
        const instance = new BotGrepo();
        // 👇 À prova de bala: expõe nos DOIS nomes
        uw.botGrepo = instance;
        window.botGrepo = instance;
        uw.multBot = instance;   // compat — outros módulos que ainda usem uw.multBot continuam a funcionar
        window.multBot = instance;
        clearInterval(_botgrepo_loader);
    }, 100);
}
